...
void init () {
	...
	for (int i = 0; i < nc; i++) {
		for (int j = 0; j < nc; j++) if (i != j) {
			/*n = getTangents(C[i], C[j], a, b);
			for (int t = 0; t < n; t++) {
				P[np++] = Point(a[t].x, a[t].y, i+1);
				P[np++] = Point(b[t].x, b[t].y, j+1);
			}
			*/
