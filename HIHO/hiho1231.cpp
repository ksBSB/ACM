...
double solve () {
	...
	for (int i = 0; i < N; i++) {
		int v = (i + 1) % N;
		sol.clear();
		int n = getLineCircleIntersection(P[i], P[v], C, t1, t2, sol);

		tmp.clear();
		tmp.push_back(0);
		if (n >= 1 && dcmp(t1) >= 0 && dcmp(t1 - 1) <= 0) {
			tmp.push_back(t1);
			ang.push_back(getAngle(sol[0]-C.o));
		}
		if (n >= 2 && dcmp(t2) >= 0 && dcmp(t2 - 1) <= 0) {
			tmp.push_back(t2);
			ang.push_back(getAngle(sol[1]-C.o));
		}
		tmp.push_back(1);

		for (int j = 1; j < tmp.size(); j++) {
			Point l = P[i] + (P[v] - P[i]) * tmp[j-1];
			Point r = P[i] + (P[v] - P[i]) * tmp[j];
			Point m = (l + r) / 2;
			if (dcmp(getLength(m-C.o) - C.r) < 0)
				ret += getLength(r-l);
		}
	}
	...
	int n = unique(ang.begin(), ang.end()) - ang.begin();

	for (int i = 0; i < n - 1; i++) {
		Point m = C.point((ang[i] + ang[i+1]) / 2);
		if (isPointInPolygon(m, P, N) < 0)
			sum += (ang[i+1] - ang[i]);
	}
	//printf("%lf %lf %lf\n", ret, sum, ret + C.r * sum);
	return ret + C.r * sum;
