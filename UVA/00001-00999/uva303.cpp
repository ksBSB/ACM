...
double handle (Point u, Point v) {

	Point o;
	for (int i = 0; i < N; i++) {
		getIntersection(u, v-u, P[i], Vector(0, 1), o);
		if (dcmp(o.y-P[i].y) > 0 || dcmp(o.y-Q[i].y) < 0) {
			if (i == 0) return P[0].x;
			double x1, x2;
			if (getIntersection(u, v-u, P[i-1], P[i]-P[i-1], o))
				x1 = o.x;
			else
				x1 = P[i-1].x;

			if (getIntersection(u, v-u, Q[i-1], Q[i]-Q[i-1], o))
				x2 = o.x;
			else
				x2 = P[i-1].x;
			return max(x1, x2);
...
void solve () {
	...
	for (int i = 0; i < N; i++) {
		for (int j = i + 1; j < N; j++) {
			ans = max(ans, handle(P[i], Q[j]));
			ans = max(ans, handle(Q[i], P[j]));
		}
	}
...
int main () {
	while (scanf("%d", &N) == 1 && N) {
		...
		for (int i = 0; i < N; i++) Q[i] = Point(P[i].x, P[i].y-1);
