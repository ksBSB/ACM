...
void solve (Point o, Point s, double r) {
	...
	for (int i = 0; i < N; i++) {
		double d = getLength(P[i] - o);
		if (dcmp(d - r) > 0) continue;
		double k = getAngle(s - o, P[i] - o);
		if (dcmp((s-o)*(P[i]-o)) < 0) k = 2 * pi - k;

		if (dcmp(k - rad) == 0 && (idx == -1 || dcmp(getLength(P[idx]-o) - getLength(P[i]-o)) < 0))
			idx = i;
	...
	if (dcmp(o.y + r) <= 0) {
		if (idx == -1) {
			E = 1, ans = rad * r; // 达不到水平线并且没有下一个折点，无限绕圈，最后周期为该圆的周长
			return;
		}
	} else {
		Point t = Point(o.x + sqrt(r * r - o.y * o.y), 0);	
		double k = getAngle(s - o, t - o);
		if (dcmp((s-o)*(t-o)) < 0) k = 2 * pi - k;
	...
	if (idx != -1) {
		rad = getAngle(s - o, P[idx] - o);
		solve(P[idx], o + rotate(s-o, rad), r - getLength(P[idx]-o));
		if (E == 0)
			ans += 2 * rad * r;
	} else
		ans = 2 * rad * r;
