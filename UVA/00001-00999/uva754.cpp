...
int handle (Point u) {
	int ret = 0;
	for (int i = 0; i < N; i++) {
		if (haveIntersection(O, u, L[i], R[i]))
			ret++;
	}
	return ret;
}
...
int solve () {
	...
	for (int i = 0; i < 2; i++) {
		for (int j = 1; j < X[i].size(); j++)
			ret = min(ret, handle(Point(i == 0 ? 0 : 100, (X[i][j]+X[i][j-1])/2)));
		for (int j = 1; j < Y[i].size(); j++)
			ret = min(ret, handle(Point((Y[i][j]+Y[i][j-1])/2, i == 0 ? 0 : 100)));
	}
	return ret;
}
