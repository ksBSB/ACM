...
bool judge (Point o) {
	...
	for (int k = 0; k < 4; k++) {
		...
		int ret = 0;
		for (int i = 0; i < N; i++) {
			//if (haveIntersection(o, u, P[i], P[i+1])) {
			if (getIntersection (o, u-o, P[i], P[i+1]-P[i], t)) {
				if ((onSegment(t, P[i], P[i+1]) || t == P[i]) && onSegment(t, o, u))
					ret++;
			}
		}
		if (ret&1)
...
double contribute (Point o) {

	for (int i = 0; i < N; i++) if (o == P[i]) {
		int u = (i - 1 + N) % N, v = (i + 1) % N;
		return getAngle(P[u] - P[i], P[v] - P[i]);
	}
	...
	for (int i = 0; i < N; i++) {
		sol.clear();
		if (1 <= getLineCircleIntersection(P[i], P[i+1], Circle(o, 0.5), a, b, sol)) {
			//for (int k = 0; k < sol.size(); k++) {
			if ((onSegment(sol[0], P[i], P[i+1])) && !onSegment(o, P[i], P[i+1]))
				return 0;
			//}
		}
	}
...
double solve () {
	...
	int maY = 0, miY = maxn;
	for (int i = 0; i < N; i++) {
		maX = max((double)maX, P[i].x);
		miX = min((double)miX, P[i].x);
		maY = max((double)maY, P[i].y);
		miY = min((double)miY, P[i].y);
	}

	for (int x = miX; x <= maX; x++) {
		for (int y = miY; y <= maY; y++)
			rad += contribute(Point(x, y));
	}
	/*
