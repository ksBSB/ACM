...
int main () {
	...
	while (scanf("%d", &N) == 1 && N) {
		...
		for (int s = 0; s < (1<<N); s++) {
			...
			if (tmpValue > ansValue) continue;

			double len = 0;
			n = getConvexHull(T, n, ch);
			ch[n] = ch[0];
			for (int i = 0; i < n; i++)
				len += getLength(ch[i+1] - ch[i]);

			if (dcmp(wood - len) >= 0) {
				if (tmpValue < ansValue || (tmpValue == ansValue && bitcount(s) < bitcount(ansState))) {
					ansValue = tmpValue;
					ansState = s;
					extra = wood - len;
				}
		...
		printf("Forest %d\nCut these trees:", ++cas);
		for (int i = 0; i < N; i++) if (ansState&(1<<i)) printf(" %d", i + 1);
		printf("\nExtra wood: %.2lf\n", extra);
	}
	return 0;
}
