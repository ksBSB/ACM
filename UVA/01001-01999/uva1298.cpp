	...
	while (scanf("%d", &N) == 1 && N) {
		...
		for (int i = 0; i < N; i++) {
			...
			for (int j = 0; j < N; j++) if (i != j) {
				...
				Point t;
				Vector v(b, -a);
				if (fabs(a) > fabs(b)) t = Point(-c/a, 0);
				else t = Point(0, -c/b);
			...
			if (ok) {
				L[lc++] = DirLine(Point(0, 0), Vector(0, -1));
				L[lc++] = DirLine(Point(0, 0), Vector(1, 0));
				L[lc++] = DirLine(Point(0, 1), Vector(-1, 1));
				if (!halfPlaneIntersection(L, lc, poly)) ok = 0;
			}
			printf("%s\n", ok ? "Yes" : "No");
		}
	}
