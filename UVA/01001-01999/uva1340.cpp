...
struct PSLG {
	...
	int n, m, cnt;
	double xi[maxp], yi[maxp];
	vector<Edge> edges;
	vector<int> g[maxp];

	int vis[maxp * 2], left[maxp * 2], prev[maxp * 2];
	...
	double getAngle(int u, int v) {
		return atan2(yi[v]-yi[u], xi[v]-xi[u]);
	}
	...
	void addEdge (int u, int v) {
		edges.push_back(Edge(u, v, getAngle(u,v)));
		edges.push_back(Edge(v, u, getAngle(v,u)));
			...
			for (int i = 0; i < g[u].size(); i++) {
				...
				if (!vis[e]) {
					...
					while (true) {
						...
						left[e] = cnt;
						int d = edges[e].u;
						poly.push_back(Point(xi[d], yi[d]));
						e = prev[e^1];
						if (e == g[u][i]) break;
					}
...
int find(Point u) { return lower_bound(V, V + C, u) - V;}
...
void init () {
	...
	for (int i = 0; i < N; i++) {
		for (int j = i+1; j < N; j++) {
			if (haveIntersection(P[i], P[i+1], P[j], P[j+1])) {
				getIntersection(P[i], P[i+1]-P[i], P[j], P[j+1]-P[j], tmp);
				V[C++] = tmp;
				dist[i].push_back(getLength(tmp-P[i]));
				dist[j].push_back(getLength(tmp-P[j]));
			}
		}
	}
	...
	for (int i = 0; i < N; i++) {
		...
		dist[i].push_back(0);
		dist[i].push_back(getLength(P[i+1]-P[i]));
...
int main () {
	while (scanf("%d", &N) == 1 && N) {
		...
		Polygon poly;
		for (int i = 0; i < plane.faces.size(); i++) {
			if (plane.area[i] < 0) {
				poly = plane.faces[i];
