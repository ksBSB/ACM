...
bool gao(Point a, Point b, Point c, Point d) {
	double k1 = (a-b)*(a-c), k2 = (a-b)*(a-d);
	if (dcmp(k1) == 0 || dcmp(k2) == 0) return true;
	if (k1*k2 < -eps) return true;
	return false;
}
...
double solve (Point a, Point b) {
	...
	for (int i = 0; i < N; i++) {
		if (getIntersection(a, b-a, P[i], P[i+1]-P[i], t)) {
			if (onSegment(t, P[i], P[i+1]) || t == P[i] || t == P[i+1])
				s.push_back(t);
		} else if (dcmp((a-b)*(a-P[i])) == 0 && dcmp((a-b)*(a-P[i+1])) == 0) {
			s.push_back(P[i]); 
			s.push_back(P[i+1]);
		}
	}
	...
	for (int i = 1; i < s.size(); i++) {
		...
		if (isPointInPolygon (k, P, N) <= 0)
			ans += getLength(s[i] - s[i-1]);
	}
	return ans;
}
