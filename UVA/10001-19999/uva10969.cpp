...
int main () {
	...
	while (cas--) {
		...
		scanf("%d", &N);
		for (int i = 1; i <= N; i++)
			scanf("%lf%lf%lf", &o[i].r, &o[i].o.x, &o[i].o.y);

		for (int i = 1; i <= N; i++) {
			vector<Point> sol;
			for (int j = 1; j <= N; j++)
				getCircleCircleIntersection (o[i], o[j], sol);

			rad[i].clear();
			rad[i].push_back(0);
			for (int j = 0; j < sol.size(); j++)
				rad[i].push_back(getAngle(sol[j] - o[i].o));
		...
		for (int i = 1; i <= N; i++) {
			...
			for (int j = 0; j < rad[i].size(); j++) {
				...
				Point u;
				if (j) {
					u = o[i].point((rad[i][j] + rad[i][j-1]) / 2);
					corner = fabs(rad[i][j]-rad[i][j-1]);
				} else {
					u = o[i].point((rad[i][j] + rad[i][rad[i].size()-1]) / 2 + pi);
					corner = fabs(rad[i][j]-rad[i][rad[i].size()-1] + 2 * pi);
				}

				bool flag = true;
				for (int k = i + 1; k <= N; k++) {
					if (dcmp(getLength(u - o[k].o) - o[k].r) <= 0) {
						flag = false; break;
					}
				}
				if (flag)
