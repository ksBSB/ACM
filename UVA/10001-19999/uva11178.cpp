...
namespace Linear {
	...
	/* 判断点是否在线段上 */
	bool onSegment (Point p, Point a, Point b) { return dcmp(getCross(a-p, b-p)) == 0 && dcmp(getDot(a-p, b-p)); }
}
...
Point solve (Point a, Point b, Point c) {
	double corb = getAngle(a - b, c - b);
	Vector vb = rotate(c - b, corb/3);

	double corc = getAngle(a - c, b - c);
	Vector vc = rotate(b - c, -corc/3);
	Point o;
	getIntersection(b, vb, c, vc, o);
	return o;
}
