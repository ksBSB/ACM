...
double getArea(Polygon u) {
	int n = u.size();
	double area = 0;
	for (int i = 0; i < n-1; i++)
		area += (u[i]-u[0])*(u[i+1]-u[0]);
	return fabs(area)/2;
}
...
int main () {
	...
	while (scanf("%d%d%d", &N, &W, &H) == 3) {
		...
		L[M++] = DirLine(Point(0, 0), Vector(W, 0));
		L[M++] = DirLine(Point(W, 0), Vector(0, H));
		L[M++] = DirLine(Point(W, H), Vector(-W, 0));
		L[M++] = DirLine(Point(0, H), Vector(0, -H));
