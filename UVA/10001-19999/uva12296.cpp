...
double getPublicAreaToPolygon (Circle O, Polygon p) {
	if (dcmp(O.r) == 0) return 0;
	int n = p.size();
	double area = 0;
	for (int i = 0; i < n; i++) {
...
int main () {
	while (scanf("%d%d%d%d", &N, &M, &L, &W) == 4 && N + M + L + W) {
		...
		u.push_back(Point(L, 0));
		u.push_back(Point(L, W));
		u.push_back(Point(0, W));
		...
		for (int i = 0; i < N; i++) {
			...
			for (int j = 0; j < faces.size(); j++) {
				Polygon left = cutPolygon(faces[j], A, B);
				Polygon right = cutPolygon(faces[j], B, A);
		...
		while (M--) {
			...
			for (int i = 0; i < faces.size(); i++) {
				double tmp = getPublicAreaToPolygon (Circle(Point(x, y), r), faces[i]);
				if (dcmp(tmp) > 0)
					ans.push_back(getArea(faces[i]));
			}
